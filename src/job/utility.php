...
class utility extends service {
  ...
  protected static function _devuser() {
    ...
    $sql = sprintf(
      'SELECT count(*) count FROM users where name = %s',
      ...
      if ($dto = $res->dto()) {
        if ($dto->count < 1) {
          $a = [
            ...
            'password' => password_hash('Thund3rB1rd', PASSWORD_DEFAULT),
            'active' => 1,
            'admin' => 1,
          ...
          $sql = sprintf(
            ...
            $db->quote(password_hash('Thund3rB1rd', PASSWORD_DEFAULT)),
            $db->quote('Administrator')
        ...
        \sys::logger( sprintf('<u.admin@domain.tld/p.Thund3rB1rd> %s', __METHOD__));
